    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6" novalidate>
        ...
        <div class="relative group">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg class="h-5 w-5 text-teal-200 group-focus-within:text-white transition-colors"
            ...
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <div class="relative flex items-center">
                    <input id="remember_me" type="checkbox" name="remember"
                        class="w-5 h-5 border-2 border-teal-300 rounded bg-white/10 text-teal-400 shadow-sm focus:ring-teal-400 focus:ring-offset-0 transition duration-200 cursor-pointer">
                </div>
                <span
                    class="ms-3 text-sm font-medium text-teal-50 group-hover:text-white transition-colors">{{ __('Ingat Saya') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm font-medium text-teal-200 hover:text-white transition-colors underline decoration-white/30 hover:decoration-white underline-offset-4"
        ...
        <div class="mt-8">
            <button type="submit"
                ...
                <!-- Shine effect -->
                <div
                    class="absolute inset-0 w-full h-full bg-white opacity-0 group-hover:opacity-20 group-hover:translate-x-full transform -translate-x-full transition-all duration-700 skew-x-12">
                </div>
        ...
        <div class="mt-8 text-center">
            <p class="text-xs text-teal-100/50">Gunakan akun yang masih aktif untuk melanjutkan</p>
        </div>
    </form>
</x-guest-layout>