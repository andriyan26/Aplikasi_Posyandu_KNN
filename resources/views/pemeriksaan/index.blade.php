        ...
        <div class="overflow-x-auto p-4 sm:p-8">
            ...
            <div class="mt-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50 dark:bg-slate-900/30 p-4 rounded-xl border border-slate-100 dark:border-slate-700/50">
                <div class="flex items-center gap-4 order-2 md:order-1">
                    <form action="{{ route('pemeriksaan.index') }}" method="GET" class="flex items-center gap-2">
                        @if(request('search'))
                    ...
                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">
                        Menampilkan <span class="font-bold text-slate-700 dark:text-slate-200">{{ $pemeriksaans->firstItem() ?? 0 }}</span> s/d <span class="font-bold text-slate-700 dark:text-slate-200">{{ $pemeriksaans->lastItem() ?? 0 }}</span> dari <span class="font-bold text-slate-700 dark:text-slate-200">{{ $pemeriksaans->total() }}</span> Data
                    </p>
                </div>
